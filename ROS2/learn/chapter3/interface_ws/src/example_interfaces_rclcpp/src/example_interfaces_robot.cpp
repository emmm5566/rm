...
public:
  ...
  float move_distance(float distance) {
    ...
    while(fabs(target_pose_ - current_pose_) > 0.01) {
      // 每一步移动当前到目标距离的1/10
      float step = distance / fabs(distance) * fabs(target_pose_ - current_pose_) * 0.1; //distance/fabs(distance)提取符号
      current_pose_ += step;
...
//利用定时器不断发送数据，收到请求后调用机器人类的move_distance接口来移动机器人
class ExampleInterfacesRobot : public rclcpp::Node 
{
public:
  // 构造函数
	ExampleInterfacesRobot(const std::string& node_name) : Node(node_name)
	{
...
private:
  ...
  void timer_callback() {
    ...
    RCLCPP_INFO(this->get_logger(), "发布当前位置: %f", robot.get_current_pose());
    // 发布消息
